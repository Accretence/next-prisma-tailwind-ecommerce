...
export async function GET(
   ...
   try {
      ...
      console.log(`[GET_CROSS_SELL_PRODUCTS] Fetching cross-sell products for product: ${productId}`)
      
      const crossSellProducts = await getCrossSellProducts(productId, limit)
      
      console.log(`[GET_CROSS_SELL_PRODUCTS] Found ${crossSellProducts.length} cross-sell products`)

      return NextResponse.json(crossSellProducts)
   } catch (error) {
      console.error('[GET_CROSS_SELL_PRODUCTS] Error details:', error)
      return NextResponse.json(
         { error: 'Failed to fetch cross-sell products', details: error.message },
         { status: 500 }
      )
   }
}
